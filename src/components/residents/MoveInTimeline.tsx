...
import { MOVE_IN_STEPS } from "@/data/residents";
import { FadeIn } from "@/components/ui/FadeIn";
import { cn } from "@/lib/utils";

export interface MoveInTimelineProps {
  className?: string;
}

export function MoveInTimeline({ className }: MoveInTimelineProps) {
  return (
      ...
      <FadeIn>
        <div className="flex flex-col gap-2 max-w-xl">
          ...
          <h3 className="font-display font-black text-3xl sm:text-5xl lg:text-6xl text-roomly-dark tracking-tight leading-tight">
            From keys to feeling at home.
          </h3>
          <p className="text-xs sm:text-sm text-roomly-muted max-w-md">
            What your transition looks like from reservation confirmation to daily routine.
          </p>
        </div>
      </FadeIn>

      <div className="relative flex flex-col gap-8 md:gap-0 pl-6 md:pl-0 select-none">
